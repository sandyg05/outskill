...
class Config:
    ...
    default_model: str = "openai/gpt-5-mini"  # Default model - using the requested GPT-5-mini model
...
class MyntraRAG:
    ...
    def _load_api_keys(self):
        ...
        if not self.config.openrouter_api_key:
            print("Warning: OPENROUTER_API_KEY not found. LLM features will be disabled.")
...
Answer: """
    ...
    def search(self, query: str, k: int = None) -> List[Document]:
        ...
        if self.vector_store is None:
            ...
            try:
                ...
                raise ValueError("No vector store available. Please run ingest_data() first.")
    ...
    def query(self, 
        ...
        if not self.config.openrouter_api_key:
            raise ValueError("No OpenRouter API key available. Please set OPENROUTER_API_KEY")
        ...
        while True:
            try:
                ...
                if user_input.lower() == 'quit':
                    ...
                    providers = {
                        "OpenAI": [k for k in self.config.openrouter_models.keys() if k.startswith("gpt")],
                        "Anthropic": [k for k in self.config.openrouter_models.keys() if k.startswith("claude")],
                        "Google": [k for k in self.config.openrouter_models.keys() if k.startswith("gemini")],
                        "Other": [k for k in self.config.openrouter_models.keys() if not any(k.startswith(p) for p in ["gpt", "claude", "gemini"])]
                    }
                    
                    print("\nAvailable model shortcuts (examples):")
                    for provider, models in providers.items():
                        if models:
                            print(f"  {provider}: {', '.join(sorted(models)[:3])}" + (" ..." if len(models) > 3 else ""))
                    ...
                    providers = {
                        ...
                        "Anthropic": [k for k in self.config.openrouter_models.keys() if k.startswith("claude")],
                        "Google": [k for k in self.config.openrouter_models.keys() if k.startswith("gemini")],
                        "Meta": [k for k in self.config.openrouter_models.keys() if k.startswith("llama")],
                        "Mistral": [k for k in self.config.openrouter_models.keys() if k.startswith("mistral") or k.startswith("mixtral")],
                        "Other": [k for k in self.config.openrouter_models.keys() if not any(k.startswith(p) for p in ["gpt", "claude", "gemini", "llama", "mistral", "mixtral"])]
                    }
                    ...
                    for provider, models in providers.items():
                        if models:
                            ...
                            for short_name in sorted(models):
                                ...
                                current = " (current)" if full_name == self.current_model or short_name == self.current_model else ""
...
def create_sample_csv():
    ...
    sample_data = {
        'product_id': ['PROD001', 'PROD002', 'PROD003', 'PROD004', 'PROD005', 'PROD006', 'PROD007', 'PROD008'],
        'product_name': [
        ...
        'category': [
            'Men Shirts', 'Women Activewear', 'Men Shirts', 'Unisex Outerwear', 
            'Men T-Shirts', 'Women Swimwear', 'Kids Activewear', 'Men Outdoor'
        ],
        'brand': ['Nike', 'Adidas', 'Puma', 'Under Armour', 'Reebok', 'Speedo', 'Decathlon', 'Columbia'],
        'price': [2499, 1999, 1799, 3299, 1599, 2799, 999, 3999],
        'color': ['Blue', 'Pink', 'Black', 'Gray', 'White', 'Navy', 'Red', 'Khaki'],
        'size': [
            'M, L, XL', 'S, M, L', 'M, L, XL, XXL', 'S, M, L, XL', 
            'M, L, XL', 'XS, S, M, L', '8-10, 10-12, 12-14', 'M, L, XL, XXL'
        ],
        'features': [
...
def test_openrouter_connection():
    ...
    try:
        ...
        response = llm.invoke("Say 'OpenRouter connected successfully!' and nothing else.")
